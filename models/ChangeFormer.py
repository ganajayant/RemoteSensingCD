...
class Attention_dec(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        q = self.q(task_q).reshape(
            B, task_q.shape[1], self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
...
class TDecV2(nn.Module):
    ...
    def __init__(self, input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=True,
        ...
        self.pix_shuffle_conv = nn.Conv2d(
            in_channels=self.embedding_dim, out_channels=16*output_nc, kernel_size=3, stride=1, padding=1)
        self.relu = nn.ReLU()
    ...
    def forward(self, inputs1, inputs2):
        ...
        _c4_1 = self.linear_c4(c4_1).permute(0, 2, 1).reshape(
            n, -1, c4_1.shape[2], c4_1.shape[3])
        _c4_1 = resize(_c4_1, size=c1_1.size()[
                       2:], mode='bilinear', align_corners=False)
        _c4_2 = self.linear_c4(c4_2).permute(0, 2, 1).reshape(
            n, -1, c4_2.shape[2], c4_2.shape[3])
        _c4_2 = resize(_c4_2, size=c1_2.size()[
                       2:], mode='bilinear', align_corners=False)

        _c3_1 = self.linear_c3(c3_1).permute(0, 2, 1).reshape(
            n, -1, c3_1.shape[2], c3_1.shape[3])
        _c3_1 = resize(_c3_1, size=c1_1.size()[
                       2:], mode='bilinear', align_corners=False)
        _c3_2 = self.linear_c3(c3_2).permute(0, 2, 1).reshape(
            n, -1, c3_2.shape[2], c3_2.shape[3])
        _c3_2 = resize(_c3_2, size=c1_2.size()[
                       2:], mode='bilinear', align_corners=False)

        _c2_1 = self.linear_c2(c2_1).permute(0, 2, 1).reshape(
            n, -1, c2_1.shape[2], c2_1.shape[3])
        _c2_1 = resize(_c2_1, size=c1_1.size()[
                       2:], mode='bilinear', align_corners=False)
        _c2_2 = self.linear_c2(c2_2).permute(0, 2, 1).reshape(
            n, -1, c2_2.shape[2], c2_2.shape[3])
        _c2_2 = resize(_c2_2, size=c1_2.size()[
                       2:], mode='bilinear', align_corners=False)

        _c1_1 = self.linear_c1(c1_1).permute(0, 2, 1).reshape(
            n, -1, c1_1.shape[2], c1_1.shape[3])
        _c1_2 = self.linear_c1(c1_2).permute(0, 2, 1).reshape(
            n, -1, c1_2.shape[2], c1_2.shape[3])

        _c = self.linear_fuse(torch.cat([torch.abs(_c4_1-_c4_2), torch.abs(
            _c3_1-_c3_2), torch.abs(_c2_1-_c2_2), torch.abs(_c1_1-_c1_2)], dim=1))
...
class DecoderTransformer_x2(nn.Module):
    ...
    def __init__(self, input_transform='multiple_select', in_index=[0, 1, 2, 3, 4], align_corners=True,
        ...
        self.linear_c5 = MLP(input_dim=c5_in_channels,
                             embed_dim=self.embedding_dim)
        self.linear_c4 = MLP(input_dim=c4_in_channels,
                             embed_dim=self.embedding_dim)
        self.linear_c3 = MLP(input_dim=c3_in_channels,
    ...
    def forward(self, inputs1, inputs2):
        ...
        _c5_1 = self.linear_c5(c5_1).permute(0, 2, 1).reshape(
            n, -1, c5_1.shape[2], c5_1.shape[3])
        _c5_2 = self.linear_c5(c5_2).permute(0, 2, 1).reshape(
            n, -1, c5_2.shape[2], c5_2.shape[3])
        ...
        _c5_up = resize(_c5, size=c1_2.size()[
                        2:], mode='bilinear', align_corners=False)
        ...
        _c4 = self.diff_c4(torch.cat(
            (F.interpolate(_c5, scale_factor=2, mode="bilinear"), _c4_1, _c4_2), dim=1))
        p_c4 = self.make_pred_c4(_c4)  # Predicted change map at x1/16 scale
        outputs.append(p_c4)  # x1/16 scale
        _c4_up = resize(_c4, size=c1_2.size()[
                        2:], mode='bilinear', align_corners=False)
        ...
        _c3 = self.diff_c3(torch.cat(
            (F.interpolate(_c4, scale_factor=2, mode="bilinear"), _c3_1, _c3_2), dim=1))
        p_c3 = self.make_pred_c3(_c3)  # Predicted change map at x1/8 scale
        outputs.append(p_c3)  # x1/8 scale
        _c3_up = resize(_c3, size=c1_2.size()[
                        2:], mode='bilinear', align_corners=False)
        ...
        _c2 = self.diff_c2(torch.cat(
            (F.interpolate(_c3, scale_factor=2, mode="bilinear"), _c2_1, _c2_2), dim=1))
        p_c2 = self.make_pred_c2(_c2)  # Predicted change map at x1/4 scale
        outputs.append(p_c2)  # x1/4 scale
        _c2_up = resize(_c2, size=c1_2.size()[
                        2:], mode='bilinear', align_corners=False)
        ...
        _c1 = self.diff_c1(torch.cat(
            (F.interpolate(_c2, scale_factor=2, mode="bilinear"), _c1_1, _c1_2), dim=1))
        p_c1 = self.make_pred_c1(_c1)  # Predicted change map at x1/2 scale
        outputs.append(p_c1)  # x1/2 scale

        _c = self.linear_fuse(
            torch.cat((_c5_up, _c4_up, _c3_up, _c2_up, _c1), dim=1))
...
class ChangeFormerV5(nn.Module):
    ...
    def __init__(self, input_nc=3, output_nc=2, decoder_softmax=False, embed_dim=256):
        ...
        self.Tenc_x2 = EncoderTransformer_v3(img_size=256, patch_size=4, in_chans=input_nc, num_classes=output_nc, embed_dims=self.embed_dims,
                                             ...
                                             attn_drop_rate=self.attn_drop, drop_path_rate=self.drop_path_rate, norm_layer=partial(
