...
from models.losses import (FocalLoss, cross_entropy, get_alpha, mIoULoss,
                           mmIoULoss, softmax_helper)
...
class CDTrainer():
    ...
    def __init__(self, args, dataloaders):
        ...
        if args.loss == 'ce':
            ...
            # calculare class occurences
            alpha = get_alpha(dataloaders['train'])
            print(
                f"alpha-0 (no-change)={alpha[0]}, alpha-1 (change)={alpha[1]}")
            self._pxl_loss = FocalLoss(
                apply_nonlin=softmax_helper, alpha=alpha, gamma=2, smooth=1e-5)
        elif args.loss == "miou":
            ...
            self._pxl_loss = mIoULoss(
                weight=weights, size_average=True, n_classes=args.n_class).cuda()
    ...
    def _load_checkpoint(self, ckpt_name='last_ckpt.pt'):
        ...
        if os.path.exists(os.path.join(self.checkpoint_dir, ckpt_name)):
            ...
            self.net_G.load_state_dict(torch.load(
                self.args.pretrain), strict=False)
            self.net_G.to(self.device)
            self.net_G.eval()
        else:
    ...
    def _forward_pass(self, batch):
        ...
        if self.multi_scale_infer == "True":
            self.G_final_pred = torch.zeros(
                self.G_pred[-1].size()).to(self.device)
            for pred in self.G_pred:
                if pred.size(2) != self.G_pred[-1].size(2):
                    self.G_final_pred = self.G_final_pred + \
                        F.interpolate(
                            pred, size=self.G_pred[-1].size(2), mode="nearest")
                else:
        ...
        if self.multi_scale_train == "True":
            ...
            for pred in self.G_pred:
                if pred.size(2) != gt.size(2):
                    temp_loss = temp_loss + self.weights[i]*self._pxl_loss(
                        pred, F.interpolate(gt, size=pred.size(2), mode="nearest"))
                else:
                    temp_loss = temp_loss + \
                        self.weights[i]*self._pxl_loss(pred, gt)
                i += 1
