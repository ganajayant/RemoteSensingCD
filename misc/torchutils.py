...
def mul_cls_acc(preds, targets, topk=(1,)):
    ...
    with torch.no_grad():
        ...
        for i in range(C):
            ...
            # print('pred: ', pred)
            correct = correct + \
                pred.eq(target.view(-1, 1).expand_as(pred)).long()
            # print('correct: ', pred.eq(target.view(-1, 1).expand_as(pred)).long())
        n = (targets == 1).long().sum(1)  # N*1, 每张图中含有目标的数量
        # print(n)
        res = []
        for k in topk:
            acc_k = correct[:, :k].sum(1).float(
            ) / n.float()  # 每张图的平均正确率，预测正确目标数/总目标数
...
def decode_seg(label_mask, toTensor=False):
    ...
    rgb = np.zeros(
        (label_mask.shape[0], label_mask.shape[1], 3), dtype=np.float)
    r = label_mask % 6
    g = (label_mask % 36) // 6
    b = label_mask // 36
    # 归一化到[0-1]
...
def save_visuals(visuals, img_dir, name, save_one=True, iter='0'):
    ...
    for label, image in visuals.items():
        ...
        for j in range(N):
            ...
            name_ = name_.split(".")[0]
            # print(name_)
            image_numpy = tensor2np(
                image[j], if_normalize=True).astype(np.uint8)
            # print(image_numpy)
            img_path = os.path.join(
                img_dir, iter+'_%s_%s.png' % (name_, label))
